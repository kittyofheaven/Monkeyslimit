...
object SmartSplitsSeeder {
    suspend fun seed(smartSplitsDao: SmartSplitsDao): List<Int> {
        ...
        val s1 = smartSplitsDao.insert(
            SmartSplits(id = 0, name = "Sushi Dinner", amountOwed = 1_740_000.0, createDate = SeedUtils.daysAgo(1))
        ).toInt()

        val s2 = smartSplitsDao.insert(
            SmartSplits(id = 0, name = "Movie Night", amountOwed = 320_000.0, createDate = SeedUtils.daysAgo(0))
        ).toInt()
